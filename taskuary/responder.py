...
SYSTEM = (
    ...
    'If the request cannot be answered by a reply alone - it needs work doing - say so in one '
    'line and stop; the owner will turn it into a task.')


def draft_reply(store, task_id: int, llm=None) -> str:
...
def draft_for_review(store, task_id: int, review_id: int, llm=None) -> str:
    """Write the draft and park it on its review, ready for approve / edit / no-reply."""
    text = draft_reply(store, task_id, llm)
