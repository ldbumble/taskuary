...
// Which colour a desk gets. The same three meanings the Timeline uses, in the same three hues.
const stateOf = (t) => {
  if (!t) return { label: "free", color: "#9aa39b" };
  if (isLive(t)) return { label: `${t.Session?.agent || t.RunAgent || "agent"} is typing`, color: ACCENT2 };
...
export default function StudioView({ onOpenTask }) {
  ...
  const scene = useMemo(() => {
    ...
    // Four corners to stand in. A rotation is a coordinate remap, not a 3D engine.
    const map = [(x, y) => [x, y], (x, y) => [y, GX - x],
      (x, y) => [GX - x, GY - y], (x, y) => [GY - y, x]][rot];
    ...
    const oy = H / 2 - (Math.min(...ys) + Math.max(...ys)) / 2 - 24;
    ...
    const dx = GX - 2.0;
    poly(BGZ, pts(P(dx, 0, 0), P(dx + 1.1, 0, 0), P(dx + 1.1, 0, 104), P(dx, 0, 104)), "#aeb5a7");
    poly(BGZ, pts(P(dx + 0.11, 0, 0), P(dx + 0.99, 0, 0), P(dx + 0.99, 0, 97), P(dx + 0.11, 0, 97)), "#f7fbf4");
    poly(BGZ, pts(P(dx + 0.11, 0, 0), P(dx + 0.99, 0, 0), P(dx + 1.3, 1.6, 0), P(dx - 0.2, 1.6, 0)), "#eef3e6");
    ...
    const person = (x, y, s, mode, ph) => {
      ...
      if (mode === "walk") {                                  // legs only show when standing up
        rect(z, cx - 7 + step * 0.5, cy - 9, 6, 12, 3, "#4a544c");
        rect(z, cx + 1 - step * 0.5, cy - 9, 6, 12, 3, "#4a544c");
      }
      ...
      oval(z + 0.07, cx - 3.6, cy - 39, 1.5, 2, "#26302a");
      oval(z + 0.07, cx + 3.6, cy - 39, 1.5, 2, "#26302a");
    };
    ...
    const tags = desks.map((t, i) => {
      ...
      const z = box(gx, gy, 2.0, 1.1, DH, "#cfd5c8", "#9aa294", "#aeb5a7");
      const mx = gx + 0.6, mw = 0.85, my = gy + 0.3;
      box(mx + 0.28, my + 0.06, 0.3, 0.24, DH + 7, "#b9c0b2", "#98a094", "#a9b0a2", 0.01);   // stand
      poly(z + 0.02, pts(P(mx - 0.05, my, DH + 6), P(mx + mw + 0.05, my, DH + 6),
        P(mx + mw + 0.05, my, DH + 40), P(mx - 0.05, my, DH + 40)), "#2b3630");
      poly(z + 0.03, pts(P(mx, my, DH + 9), P(mx + mw, my, DH + 9),
        P(mx + mw, my, DH + 37), P(mx, my, DH + 37)), live ? "#141d18" : "#dfe4da");
      if (live) for (let k = 0; k < 3; k++) {                  // code, and it scrolls while it types
      ...
      box(gx + 0.35, gy + 0.7, 0.9, 0.28, DH + 2, "#dfe4da", "#bcc3b6", "#ccd2c5", 0.02);    // keyboard
      if (t) person(gx + 1.0, gy - 0.55, SKINS[i % SKINS.length], live ? "type" : "sit", frame);
    ...
    const ports = sources.slice(0, Math.min(4, room)).map((s, i) => {
      ...
      poly(BGZ, pts(P(x + 0.08, 0, 61), P(x + 1.02, 0, 61), P(x + 1.02, 0, 81), P(x + 0.08, 0, 81)), i === 0 ? "#e4efe8" : "#eef1eb");
      const m = P(x + 0.55, 0, 71);
      prims.push({ k: "l", z: BGZ + 1, d: `M ${m[0] + 80} ${m[1] - 170} Q ${m[0] + 44} ${m[1] - 78} ${m[0]} ${m[1]}`,
        stroke: i === 0 ? ACCENT : "#c2cabb", o: i === 0 ? 0.75 : 0.4 });
  ...
  }, [rot, desks, sources, queue.length, frame]);
  ...
  return (
    <Box sx={{ position: "relative", width: "100%", height: "calc(100vh - 190px)", minHeight: 520, overflow: "hidden" }}>
      ...
      {scene.ports.map((p) => (
        <Box key={p.name + p.x} sx={{ position: "absolute", left: pc(p.x, W), top: pc(p.y, H),
