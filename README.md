...
| `COUNSEL.md` | how the assistant briefs **you** — privately, on every message a model judged: what it's really about, what it would do, what to get ahead of, the task nobody named yet (one click makes it real); invites get a prep note. And on its own clock it **posts on the Timeline** — the reply you never heard back on (one click drafts the chase), the meeting ahead with what came before it, the task gone quiet, its own ideas — never the same line twice (Settings → Assistant). Written against what the hub already knows: this sender's recent mail, what you last wrote them, the topic elsewhere, open tasks, your calendar — and reply drafts read that same history | the assistant brief (cheap model), reply drafts |
