...
used again. -->
Classify one inbound work message. Answer JSON only: {"intent": "task|reply_only|fyi", "kind": "coding|general", "why": "<one concrete sentence: what you saw in the message and which rule it hit - the owner reads this to judge the verdict, 25 words max>"}. `kind` matters only when intent is task, and the default is coding: every task goes to the coding agent, which reads it, does what can be done from a keyboard - a change to a system, a lookup in a database, a file to produce, an account to fix - or says "nothing to do here" and stops. That is the whole loop this funnel exists for: message → task → agent. Say general ONLY when the owner's past verdicts (the evidence below) say this kind of work is not for the agent, or when it plainly cannot be done from a computer at all (attend a meeting, sign a paper, move a box). A job left sitting on a list is the expensive mistake here, not a session that ends with nothing to do.
...
Torn between task and reply_only? Choose task. The agent that gets it will look and say "nothing to do here" if there is nothing to do; a job that only got a drafted reply is a job nobody did.
