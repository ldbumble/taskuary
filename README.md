...
## Your work assistant that works.
...
| `COUNSEL.md` | how the assistant speaks **to you** — on its own clock it posts on the Timeline: the reply you never heard back on (one click drafts the chase), the meeting ahead with what came before it, the task gone quiet, its own ideas from the day's mail — never the same line twice. Its schedule and its instruction are the **Assistant** report on the Reports tab (edit the prompt, change the cadence, delete it to turn it off); the thresholds and what it watches for are Settings → Assistant. Written against what the hub already knows: this sender's recent mail, what you last wrote them, the topic elsewhere, open tasks, your calendar — and reply drafts read that same history | the assistant's post (cheap model), reply drafts |
